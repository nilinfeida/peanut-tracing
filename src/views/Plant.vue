...
<div>
  ...
  <el-card style="margin-bottom: 10px">
    <div class="pd-10">
      <el-input style="width: 200px;margin-right: 10px" v-model="cropName" suffix-icon="el-icon-search" placeholder="请输入作物名称"  @keyup.enter.native="search"></el-input>
      <el-input style="width: 200px;margin-right: 10px" v-model="id" suffix-icon="el-icon-search" placeholder="请输入种植环节id"  @keyup.enter.native="search"></el-input>
  ...
  <el-card>
    <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="cropName" label="作物名" width="180">
      </el-table-column>
      <el-table-column prop="picUrl" label="作物图片" width="180">
      ...
      <el-table-column prop="sowingTime" label="播种时间" width="150">
      </el-table-column>
  ...
  <!--弹窗表单-->
  <el-dialog title="作物信息" :visible.sync="dialogFormVisible" width="30%" center  @close="resetForm" >
    <el-form :model="form" size="small" label-width="80px"  ref="form" :rules="rules">
      <el-form-item label="作物图片" prop="picUrl">
        <el-upload
...
export default {
  ...
  methods:{
    ...
    deleteBatch(){
      ...
      this.request.post("/plant/delBatch",ids).then(res => {
        this.$message({
          type: 'success',
          message: "删除成功"
        });
    ...
    deleteData(plant) {
      this.$confirm(`您确定删除${plant.cropName}种植信息?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.request.delete("/plant/"+plant.id).then(res => {
          if(res.code==200){
            this.$message({
              type: 'success',
              message: "删除成功"
    ...
    save(){
      this.$refs.form.validate((valid) => {
        if(valid){
          this.request.post("/plant",this.form).then(res=>{
            if(res.code==200){
              // 成功提示
              this.$message({
                message: "种植信息更新成功",
    ...
    modData(row){
      this.request.get("/plant/"+row.id).then(res=>{
        if(res.code==200){
          res=res.data
          this.dialogFormVisible = true
          this.form=res
