...
<div>
  ...
  <el-card style="margin-bottom: 10px">
    <div class="pd-10">
      <el-input style="width: 200px;margin-right: 10px" v-model="dealer" suffix-icon="el-icon-search" placeholder="请输入销售商名称"  @keyup.enter.native="search"></el-input>
      <el-input style="width: 200px;margin-right: 10px" v-model="id" suffix-icon="el-icon-search" placeholder="请输入销售id"  @keyup.enter.native="search"></el-input>
  ...
  <el-card>
    <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="dealer" label="经销商" width="150" >
      </el-table-column>
      <el-table-column prop="businessLicense" label="营业执照" width="150">
      </el-table-column>
      <el-table-column prop="phone" label="联系电话" width="150">
      </el-table-column>
      <el-table-column prop="address" label="经销商地址" width="150">
      </el-table-column>
  ...
  <el-dialog title="物流信息" :visible.sync="dialogFormVisible" width="30%" center  @close="resetForm" >
    <el-form :model="form" size="small" label-width="80px"  ref="form" :rules="rules">
      <el-form-item label="经销商" prop="dealer">
        <el-input v-model="form.dealer" autocomplete="off" ></el-input>
      </el-form-item>
      <el-form-item label="营业执照" prop="businessLicense">
        <el-input   v-model="form.businessLicense" autocomplete="off" ></el-input>
      </el-form-item>
  ...
  <el-dialog title="溯源绑定" :visible.sync="dialogTableVisible" width="50%" center   >
    ...
    <el-card>
      <el-table :data="dTableData" stripe @selection-change="dHandleSelectionChange">
        ...
        <el-table-column prop="saleId" label="销售ID" width="100">
        </el-table-column>
        <el-table-column prop="upload" label="区块链" width="75">
          <template slot-scope="scope">
            <el-tag
...
export default {
  ...
  methods: {
    ...
    deleteBatch() {
      ...
      this.request.post("/sale/delBatch", ids).then(res => {
        this.$message({
          type: 'success',
          message: "删除成功"
        });
    ...
    deleteData(sale) {
      this.$confirm(`您确定删除该条物流信息?`, '提示', {
        ...
        this.request.delete("/sale/" + sale.id).then(res => {
          if (res.code == 200) {
            this.$message({
              type: 'success',
              message: "删除成功"
    ...
    save() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.request.post("/sale", this.form).then(res => {
            if (res.code == 200) {
              // 成功提示
              this.$message({
                message: "销售信息更新成功",
    ...
    traceSave(){
      ...
      console.log(ids)
      this.request.post("/sale/trace/"+this.rId, ids).then(res => {
        if (res.code == 200) {
          // 成功提示
          this.$message({
            message: "溯源信息绑定成功",
    ...
    modData(row) {
      this.request.get("/sale/" + row.id).then(res => {
        if (res.code == 200) {
          res = res.data
          this.dialogFormVisible = true
          this.form = res
